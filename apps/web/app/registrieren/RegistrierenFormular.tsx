 ...
 */
export function RegistrierenFormular({ weiter }: { weiter: string }) {
  const eingeladen = weiter !== "/";
    ...
    <Einstieg
      ...
      fuss={
        <p className={styles.hinweis}>
          {/* Mit Einladung stimmt der Satz nicht: die Einladung IST das
              Hinzufuegen (Testnotiz 25.09., #6). */}
          {eingeladen
            ? "Danach nimmst du die Einladung an."
    ...
    >
      <form action={registrierenAction}>
        ...
        <div className={styles.links}>
          <a href={eingeladen ? `/login?weiter=${encodeURIComponent(weiter)}` : "/login"}>
