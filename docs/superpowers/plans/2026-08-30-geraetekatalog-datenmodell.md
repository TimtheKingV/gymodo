...
- **Studio-Konsistenz bei Verknüpfungstabellen:** Wo eine Zeile auf zwei studio-gebundene Tabellen verweist (`equipment_model_exercises`, `machines`), erzwingt die INSERT/UPDATE-Policy per Join, dass beide Seiten demselben Studio angehören — nicht nur Anwendungscode. Eine studioübergreifende Verknüpfung ist damit auf Datenbankebene unmöglich, nicht nur unerwünscht.
