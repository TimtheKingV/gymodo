...
- **Eingrenzen geht bei keinem der beiden Suite-Befehle über `--`.** `pnpm` reicht das `--` wörtlich an
  das darunterliegende Werkzeug durch, das es als Ende der Flags deutet. Richtig:
  `npx vitest run --config vitest.config.ts <datei>` und `pnpm exec playwright test <datei>`.
  Dreimal auf diesem Bauabschnitt aufgefallen — zuletzt am Portargument von `next dev`.
...
Run: `grep -rn "und eine Nachricht" docs/superpowers/design/`
Expected: keine Treffer.

**Nicht ueber `docs/` insgesamt suchen.** Die Vorabnotiz, die Spec und dieser Plan zitieren den alten Satz, um die Entscheidung festzuhalten, ihn zu entfernen. Wer ihn dort mit tilgt, loescht die Begruendung und behaelt nur das Ergebnis.
...
Run: `E2E_PORT=3011 pnpm exec playwright test e2e/kurse.spec.ts`

**Nicht `pnpm test:e2e -- e2e/kurse.spec.ts`** -- das grenzt nicht ein. pnpm reicht das `--` woertlich
an Playwright durch, und alle 32 Tests laufen. Dieselbe Falle wie bei `pnpm test:integration`, und
dieselbe wie beim Portargument von `next dev`: gemessen, nicht vermutet.

**Und der eigene Port ist Pflicht, nicht Bequemlichkeit.** `reuseExistingServer` ist lokal `true`; haelt
eine Parallelsession bereits 3000, laeuft der Gang gegen deren Code. `playwright.config.ts` liest
deshalb `E2E_PORT`, Vorgabe 3000.
...
- [ ] `pnpm test:e2e` — grün
- [ ] `grep -rn "und eine Nachricht" docs/superpowers/design/` — keine Treffer.
      **Nicht** ueber `docs/` insgesamt: Vorabnotiz, Spec und dieser Plan zitieren den alten Satz, um die Entscheidung zu dokumentieren, ihn zu entfernen. Wer ihn dort tilgt, loescht die Begruendung und behaelt nur das Ergebnis.
