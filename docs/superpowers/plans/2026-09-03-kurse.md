...
| `portal/[studioId]/(schreibtisch)/kurse/KurseActions.tsx` **(neu)** | Die Client-Teile der Kursseiten (Absage mit Bestätigung, Teilnehmer entfernen) |
...
**Warum `kurse-actions.ts` und nicht `portal/actions.ts`:** `portal/actions.ts` ist bereits 20 Importe breit und liefert überall `ActionResult`. Die Kursseiten brauchen an zwei Stellen mehr als `ok` — beim Anlegen einer Serie die Zahl der entstandenen Termine, beim Stornieren den Grund einer Absage (`deadline`, `not_booked`). Beides in die bestehende Datei zu zwingen hieße, ihr Rückgabeformat für alle zwanzig Aufrufstellen aufzuweichen. Was zusammen wächst, liegt zusammen — dieselbe Erwägung, aus der `einrichten/actions.ts` entstanden ist.
...
In `packages/domain/src/index.ts`, in der alphabetischen Nachbarschaft der übrigen `export *`:
...
An `packages/domain/src/courses.ts` anfügen:
...
describe("wochenFenster", () => {
  it("ohne Anker liegt das Fenster um heute und beginnt am Montag", () => {
    const fenster = wochenFenster(undefined, BERLIN);
    expect(new Date(fenster.bis).getTime() - new Date(fenster.von).getTime()).toBe(
      7 * 24 * 3_600_000,
    );
  });
  ...
  it("die Woche mit der Zeitumstellung ist trotzdem sieben Tage lang -- gemessen an der Wanduhr", () => {
    const fenster = wochenFenster("2026-10-26", BERLIN);
    expect(fenster.titel).toBe("Mo., 26. Oktober – So., 1. November 2026");
    // Auf dem Zeitstrahl sind es 169 Stunden, nicht 168.
    const stunden =
...
export type KursErgebnis = { ok: true } | { ok: false; error: string };

function alsErgebnis(fehler: unknown): KursErgebnis {
...
```json
    "./serie": {
      "types": "./dist/serie.d.ts",
      "import": "./src/serie.ts"
    }
```

Richte dich beim Format nach dem bestehenden Eintrag für `./tag-scan` — er ist das Vorbild und trägt die richtigen Pfade für dieses Setup.
...
- Anlegen: `apps/web/app/portal/[studioId]/(schreibtisch)/kurse/KurseActions.tsx`

**Schnittstellen:**
- Nutzt: `listCourseWeek`, `listCourseParticipants` (Aufgabe 6); `terminSpeichernAction`, `terminAbsagenAction`, `teilnehmerEntfernenAction` (Aufgabe 7)
- Liefert: `AbsageKarte`, `TeilnehmerZeile` (Client-Komponenten)

- [ ] **Schritt 1: Die Client-Teile anlegen**

`apps/web/app/portal/[studioId]/(schreibtisch)/kurse/KurseActions.tsx`:

```tsx
"use client";

import { useState, useTransition } from "react";
import type { KursErgebnis } from "../../kurse-actions";
// Zwei Ebenen hoch ist [studioId], drei sind app/portal -- dort liegt das
// Stylesheet. Vorbild: (schreibtisch)/einstellungen/page.tsx.
import styles from "../../../portal.module.css";

/**
 * Absagen und Entfernen sind bestaetigte Handlungen -- beide wirken auf
 * Leute, die sich auf etwas verlassen haben. Bewusst KEIN window.confirm:
 * ein Browserdialog sagt nicht, was passiert, und der Text darunter tut
 * es.
 */

export function AbsageKarte({
  absagen,
  bereitsAbgesagt,
}: {
  absagen: () => Promise<KursErgebnis>;
  bereitsAbgesagt: boolean;
}) {
  ...
  return (
    <div>
      <p>
        Der Termin bleibt sichtbar und wird als abgesagt gekennzeichnet. Angemeldete
        Mitglieder sehen, dass er ausfällt.
      </p>
      {sicher ? (
        <>
          <button
            ...
            onClick={() =>
              starte(async () => {
                const ergebnis = await absagen();
                if (!ergebnis.ok) setFehler(ergebnis.error);
              })
            }
          >
            {laeuft ? "Wird abgesagt …" : "Ja, Termin absagen"}
          </button>
          <button type="button" onClick={() => setSicher(false)} disabled={laeuft}>
            Doch nicht
          </button>
        </>
      ) : (
        <button type="button" onClick={() => setSicher(true)}>
          Termin absagen
        </button>
      )}
      {fehler !== null && <p className={styles.error}>{fehler}</p>}
    </div>
  );
}

export function TeilnehmerZeile({
  email,
  zusatz,
  entfernen,
}: {
  email: string;
  zusatz: string;
  entfernen: () => Promise<KursErgebnis>;
}) {
  const [fehler, setFehler] = useState<string | null>(null);
  const [laeuft, starte] = useTransition();
  ...
  return (
    <li>
      <span>{email}</span>
      <span className={styles.navItemMeta}>{zusatz}</span>
      <button
        type="button"
        disabled={laeuft}
        onClick={() =>
...
import styles from "../../../../../portal.module.css";
import { AbsageKarte, TeilnehmerZeile } from "../../KurseActions";
import { uhrzeit } from "../../woche";
...
export default async function TerminPage({
  ...
  // liefert -- deshalb auch hier, mit einem Fenster um den Termin herum.
    ...
    <main className={styles.content}>
      ...
      <section className={styles.section}>
        ...
        >
          <Feld
            name="startsAt"
            label="Beginn (ISO 8601 mit Zonenangabe)"
          ...
          <ul>
            {gebucht.map((person) => (
              <TeilnehmerZeile
                ...
                entfernen={teilnehmerEntfernenAction.bind(
                  null,
                  studioId,
                  sessionId,
                  person.userId,
          ...
          <ul>
            {wartend.map((person) => (
              <TeilnehmerZeile
                key={person.userId}
                email={person.email}
                zusatz={`Position ${person.waitlistPosition}`}
                entfernen={teilnehmerEntfernenAction.bind(
                  null,
                  studioId,
                  sessionId,
      ...
      <section className={styles.section}>
        <h2>Absagen</h2>
        <AbsageKarte
          absagen={terminAbsagenAction.bind(null, studioId, sessionId)}
          bereitsAbgesagt={termin.status === "cancelled"}
        />
      </section>
    </main>
  );
...
```bash
git add apps/web/app/portal
git commit -m "feat(web): Termin-Detail -- Teilnehmerliste, Nachrueck-Vermerk und die Absage mit Bestaetigung"
```
...
describe("GET /api/v1/me/courses", () => {
  ...
  it("ohne studio-Parameter 400", async () => {
...
Sollte `errorResponse` andere Codes auf Statuscodes abbilden als hier angenommen, richte dich nach `apps/web/lib/api/respond.ts` — der Test prüft Statuscodes, und die Datei entscheidet sie.
