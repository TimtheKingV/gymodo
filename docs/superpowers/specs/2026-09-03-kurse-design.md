...
- **Wer die Trainerauswahl baut, entscheidet zwei Dinge mit.** In dieser Fassung setzt kein Bildschirm `instructor_user_id`, und die **Update**-Pfade fassen die Spalte deshalb nicht an — sie nehmen sie nicht einmal entgegen. Der Grund ist ein Fehler, den der Review gefunden hat: die Server Actions lasen ein Formularfeld, das keine Seite rendert, und schrieben die Spalte damit bei jedem Speichern auf `NULL`. Folgenlos, solange nichts sie setzt — eine Falle, sobald etwas sie setzt.

  Der Bildschirm, der die Auswahl später anbietet, erbt daher zwei offene Fragen. **Erstens:** was bedeutet „nichts ausgewählt" — die Zuordnung löschen oder sie unverändert lassen? **Zweitens:** `course_week` liefert `instructor_name`, aber nicht `instructor_user_id`. Ein Bildschirm, der die Zuordnung anzeigen oder durchreichen soll, braucht die Kennung also zuerst im Rückgabewert der Funktion — das ist eine Migration, keine Oberflächenarbeit.
